...
func NewLifecycle(
	name string,
	startFunc LifecycleFunc,
	workTriggerAndStopSignal chan bool, // true = work, false = stop
	workFunc LifecycleFunc,
	workErrorHandlerFunc ErrorHandlerFunc,
	stopFunc LifecycleFunc,
) *Lifecycle {
	l := Lifecycle{
		...
		stopResponse:             make(chan bool),
		name:                     name,
		startFunc:                startFunc,
		workFunc:                 workFunc,
		workErrorHandlerFunc:     workErrorHandlerFunc,
		stopFunc:                 stopFunc,
		workTriggerAndStopSignal: workTriggerAndStopSignal,
	}
...
exit:
	for {
		if l.workTriggerAndStopSignal != nil {
			// this path blocks on either of these
			select {
			case <-l.stopRequest:
				break exit
			...
			// this path checks but doesn't block
			select {
			case <-l.stopRequest:
				break exit
			default:
